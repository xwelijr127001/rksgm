...
export default function Join() {
  ...
  return <PlayerShell back="/" label="Gabung permainan"><main className="join-main">
    <div className="join-heading">{TOKOH.missRaksa.aktif ? <KarakterTokoh tokoh="missRaksa" className="join-tokoh" tinggi={116} teks={step === 0 ? TOKOH.missRaksa.sapaan.gabungKode : TOKOH.missRaksa.sapaan.gabungKenalan} /> : <Raki size={86} mood="sapa" />}<span className="eyebrow">{step === 0 ? 'LANGKAH 1 DARI 2' : 'TINGGAL SATU LANGKAH'}</span><h1>{step === 0 ? 'Punya kode main?' : 'Kenalan dulu, yuk.'}</h1><p>{step === 0 ? 'Lihat 4 karakter di layar acara.' : 'Isi nama dan pilih karakter favoritmu.'}</p></div>
